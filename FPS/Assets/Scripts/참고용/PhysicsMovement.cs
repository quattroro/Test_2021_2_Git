...
namespace Rito.CharacterControl
{
    public class PhysicsMovement : MonoBehaviour, IMovement3D
    {
        ...
        ***********************************************************************/
        ...
        private float _capsuleRadiusDiff;//캡슐콜라이더
        ...
        ***********************************************************************/
        ...
        void IMovement3D.SetMovement(in Vector3 worldMoveDir, bool isRunning)//움직일 방향과 크기하고 현재 달리고 있는지를 받아와서 변수들을 초기화
        {
            Current.worldMoveDir = worldMoveDir;
            State.isMoving = worldMoveDir.sqrMagnitude > 0.01f;//움직임이 0.1보다 크면 움직이는 상태
            State.isRunning = isRunning;
        }


        //점프가 가능한지등을 확인하고 점프를 시켜준다.
        ...
        {
            SetOutOfControl(time);//지정된 시간 동안 컨트롤 불가
            Com.rBody.AddForce(force, ForceMode.Impulse);//넣어준 힘에 반대 방향으로 힘을 가해준다.
        }
        ...
        ***********************************************************************/
        ...
        private void CheckGround()
        {
            ...
            //
            bool cast =
                Physics.SphereCast(CapsuleBottomCenterPoint, _castRadius, Vector3.down, out var hit, COption.groundCheckDistance, COption.groundLayerMask, QueryTriggerInteraction.Ignore/*트리거 이벤트를 발생시키지 않는다.*/);
            ...
            {
                // 지면 노멀벡터 초기화
                Current.groundNormal = hit.normal;//충돌위치에서 

                // 현재 위치한 지면의 경사각 구하기(캐릭터 이동방향 고려)
                Current.groundSlopeAngle = Vector3.Angle(Current.groundNormal, Vector3.up);//지면의 각도 구함
        ...
        private void UpdatePhysics()//떨어질때 중력가속도를 붙혀준다.
        {
        ...
        private void CalculateMovements()
        {
            ...
            if (State.isGrounded || Current.groundDistance < COption.groundCheckDistance && !State.isJumping)
            {
                if (State.isMoving && !State.isForwardBlocked)
                {
                    ...
                    {
                        bool isPlus = Current.forwardSlopeAngle >= 0f;//속도가 증가하냐 감소하냐
